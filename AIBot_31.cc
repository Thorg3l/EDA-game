...
struct PLAYER_NAME : public Player {
    ...
    Pos warrior_bfs(const Unit& ally) {
        ...
        while (not possible_position.empty() and not dead) {
            ...
            if (cell(actual).type == Water and ally.water - contents.first < 5) return first_step(movements, actual, ally.pos);
    ...
    Pos car_bfs(const Unit& ally) {
        ...
        while (not possible_position.empty()) {
            ...
            else if (cell(actual).id != -1 and check_unit(cell(actual).id, ally.id, false, true)) return first_step(movements, actual, ally.pos);
