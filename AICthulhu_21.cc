...
struct PLAYER_NAME : public Player {
        ...
        if (not all_stations[i].empty()) {
          ...
          if (pos_ok(p)) {
            ...
            for (int X = 0; X < 8; ++X) {
              ...
              if (pos_ok(xx)) {
                Cell c = cell(xx);
                if (c.type != Wall and c.type != Station and c.type != Water and c.type != City and (station_dist[xx.i][xx.j] == -1 or (station_dist[xx.i][xx.j] > (station_dist[p.i][p.j]+1)))) {
                  station_dist[xx.i][xx.j] = station_dist[p.i][p.j] + 1;
              ...
              if (c.id != -1) {
                  ...
                  if (enemy.player != me()) {
                      if (enemy.type == Car) return false;
                        ...
                        if (ally.type != Car and double(enemy.water*1.0/(enemy.water+ally.water)) >= double(enemy.water*1.0/(enemy.water+ally.water))) return false;
                      }
  ...
  bool check_unit(int id_enemy, int id, bool city, bool car) {
      ...
      if (enemy.player != me() and enemy.type != Car) {
          ...
          else return (double(enemy.water*1.0/(enemy.water+ally.water)) <= double(ally.water*1.0/(enemy.water+ally.water)));
          return true;
      }
      return false;
  }
  ...
  void search_warriors(int id) {
    ...
    if (u.food-5 < m_food and m_food < 20 and m_food < round()-10) {
      survive(u,m_food,d_food,false);
      return;
    }
    else if (u.water-5 < m_water and m_water < 20 and m_food < round()-10) {
  ...
  void search_cars(int id) {
    ...
    if (not car.food) {
      ...
      for (int i = 0; i < 8; ++i) {
        Dir d_aux = Dir(i);
        Pos p_aux = p + d_aux;
        if (pos_ok(p_aux) and station_dist[p_aux.i][p_aux.j] > 0 and min_station > station_dist[p_aux.i][p_aux.j]) {
          min_station = station_dist[p_aux.i][p_aux.j];
          d = d_aux;
        }
      }
      if (occupied.find(p+d) == occupied.end()) {
        occupied.insert(p+d);
        command(id,d);
      }
      else {
