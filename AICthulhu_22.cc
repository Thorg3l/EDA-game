...
struct PLAYER_NAME : public Player {
  ...
  void go_to(Pos obj, Unit u) {
      if (pos_ok(obj)) {
          ...
          for (int i = 0; i < 8; ++i) {
              if (u.pos+Dir(i) != obj and scan(u.pos+Dir(i),u) and occupied.find(u.pos+Dir(i)) == occupied.end()) {
                command(u.id, Dir(i));
                occupied.insert(u.pos+Dir(i));
                return;
              }
  ...
  void search_cars(int id) {
    ...
    if (car.food <= min_station and scan(p+d,car) and occupied.find(p+d) == occupied.end()) {
        occupied.insert(p+d);
        command(id,d);
        return;
    }
